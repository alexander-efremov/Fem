...
__pure inline static bool try_get_slope_ratio(const c_dp_t& bv, const c_dp_t& uv, double& value)
{
	...
	value = fabs((uv.y - bv.y) / (uv.x - bv.x)); // угловой коэффициент пр€мой
	if (value < 1e-12)
	{
		return false;
	}
...
__pure static double integrate_rectangle_one_cell(double* prev_dens, double py, double qy, double gx, double hx, const c_ip_t& sx, const c_ip_t& sy)
{
	...
	b = sx.y >= 0 && sy.y >= 0 ? OY_DEVICE[sy.y] : C_HY * sy.y; // Ё“ќ ѕЋќ“Ќќ—“№ — ѕ–≈ƒџƒ”ў≈√ќ —Ћќя ƒЋя ƒјЌЌќ… я„≈… »
...
__pure static double integrate_right_slant_chanel(double* prev_dens, const c_dp_t& bv, const c_dp_t& uv, bool is_rect_truncated, const c_ip_t& sx, double b, const c_ip_t& sb, const c_ip_t& sy)
{
	...
	//   ј теперь прибавим все пр€моугольные куски, которые помещаютс€ в €чейку
...
// используетс€ дл€ upper left и дл€ bottom left треугольника
// т.е. случай
// UPPERLEFTTR
//
//                  CENTRE
                                          ...
                                          double b, const c_ip_t& sb)
{
	...
	// case B: не полный пр€моугольник
	if (is_rect_trunc)
	{ // это значит, что пр€моугольник занимает не всю €чейку  
...
// определим целочисленные индексы квадратов в которых лежат верхн€€ и нижн€€ точки треугольника
// sx = (x,y) координаты квадрата в которой лежит нижн€€ точка
// sy = (x,y) координаты квадрата в которой лежит верхн€€ точка
// в случае успешной проверки, k = будет  угловой коэфициент пр€мой
...
__pure static double integrate_right_triangle_bottom_left(double* prev_dens, const c_dp_t& bv, const c_dp_t& uv)
{
	...
	while (true)
	{
		//TODO: sx.x и sx.y должны быть положительными всегда?  ажетс€ дл€ sx.x это всегда верно...
		...
		{
			// сюда попадаем и в случае когда треугольник полностью в одной €чейке лежит
...
__pure static double integrate_right_triangle_upper_left(double* prev_dens, const c_dp_t& bv, const c_dp_t& uv)
{
	...
	while (true)
	{
		...
		if (next.x - uv.x > FLT_MIN) // если следующа€ точка уже правее, чем наша гранична€ точка, то мы обработали канал
		{
...
{
	// точки должны идти в пор€дке возрастани€ y координаты, чтобы правильно отработала процедура интегрировани€		
	...
	//   ¬озможны 2 случа€ расположени€ точки пересечени€ относительно средней
	//   слева или справа.
	//   есди средн€€ точка справа от точки пересечени€
	//   обмен€ем местами  X координаты, чтобы использовать один код дл€ расчета
	c_dp_t t = y;
	if (t.x >= ip.x)
	{
		double tx = t.x;
...
{
	// TODO какой пор€док тут все таки предполагетс€? против часовой начина€ с верхней левой?	
